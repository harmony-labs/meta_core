...
impl Layer {
    ...
    /// 
    /// Note: This is a per-layer decision. Use `negotiate()` for full sync planning
    /// which handles layer dependencies (e.g., IndexMeta ships with IndexData).
    #[inline]
    pub fn should_ship(&self, peer_can_generate: bool) -> bool {
        match self.kind {
            LayerKind::Canonical => true,       // Always ship canonical data
            LayerKind::IndexMeta => true,       // Metadata always ships (small, needed for verification)
            LayerKind::Embedding => !peer_can_generate,
            LayerKind::IndexData => !peer_can_generate,
        }
    }
